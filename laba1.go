...
func saveStackToFile(stack *Stack, filename string) error {
	...
	current := stack.head
	for current != nil {
		_, err := fmt.Fprint(file, current.data) // Изменено здесь
		if err != nil {
			return err
		}
...
func saveSetToFile(set *Set, filename string) error {
	...
	for key := range set.data {
		_, err := fmt.Fprintln(file, key) // Изменено здесь
		if err != nil {
			return err
		}
	}
	...
	for {
		...
		case 1:
			...
			if set.Contains(value) {
				fmt.Println("Ошибка: Вы пытаетесь добавить элемент с существующим значением.")
			} else {
				set.Add(value)
		...
		case 2:
			...
			value, _ := reader.ReadString('\n')
			value = strings.TrimSpace(value) // Уберем пробелы и переводы строк
			if set.Contains(value) {
	...
	for {
		...
		case 1:
			...
			if _, ok := hashTable.Get(key); ok {
				fmt.Println("Ошибка: Вы пытаетесь добавить элемент с существующим ключом.")
			} else {
				hashTable.Put(key, value)
				fmt.Println("Элемент добавлен в хеш-таблицу.")
				// Сохранение данных хеш-таблицы в файл после добавления элемента
				if err := saveHashTableToFile(hashTable, "hashtable.txt"); err != nil {
